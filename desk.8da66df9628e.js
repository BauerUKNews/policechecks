import {patchSelection,markerColour,safeLink,healthState,minutesAgo,releaseFreshness,orderedGroups,collectorTime,clockDisplay,nextScrapeLabel,isForceAreaOnly} from './logic.df67e40b5cc1.js';
...
function clearDesk(){state.expanded.clear();if($('#filter-dialog').open)$('#filter-dialog').close();$('#mobile-admin-tools').hidden=true;state.rows=[];state.member=null;state.version++;state.busy=false;$('#desk').hidden=true;$('#releases').replaceChildren();$('#members').replaceChildren();if(markers)markers.clearLayers();syncResponsiveLayout();}
...
async function onSession(session){const version=++authVersion;clearDesk();state.session=session;$('#account').hidden=!session;$('#waiting').hidden=true;$('#auth').hidden=!!session&&!recovery;message('#email',session?.user.email||'');if(!session||recovery)return;
 ...
 syncResponsiveLayout();initMap();renderSelection();await loadReleases();if(member.is_admin)await loadMembers();
