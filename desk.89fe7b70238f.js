...
let accessTimer,accessPollBusy=false;
let writingUsage=null,currentDraft=null,writingBusy=false,writingMenuTrigger=null;
const drafts=new Map();
let overlayKey='',client,map,overlays,markers,recovery=false,authVersion=0,searchTimer;
...
function clearDesk(){clearTimeout(accessTimer);state.access=null;clearWriting();state.expanded.clear();if($('#filter-dialog').open)$('#filter-dialog').close();if($('#password-dialog').open)$('#password-dialog').close();$('#password-form').reset();$('#mobile-admin-tools').hidden=true;state.rows=[];state.member=null;state.version++;state.busy=false;$('#desk').hidden=true;$('#releases').replaceChildren();$('#members').replaceChildren();if(markers)markers.clearLayers();syncResponsiveLayout();}
...
async function boundedAccessRead(request){let timer;try{return await Promise.race([request,new Promise((_,reject)=>{timer=setTimeout(()=>reject(new Error('Access check timed out. Please retry.')),10000);})]);}finally{clearTimeout(timer);}}
async function membership(){const {data,error}=await boundedAccessRead(client.from('desk_members').select('*').eq('user_id',state.session.user.id).maybeSingle());if(error)throw error;return data;}
async function accessStatus(){const started=performance.now();const {data,error}=await boundedAccessRead(client.rpc('desk_my_access'));if(error)throw new Error('Access schedules could not be checked. Contact Owen.');if(typeof data?.allowed!=='boolean')throw new Error('Access could not be verified.');return {...data,checkElapsed:performance.now()-started};}
function showAccessMessage(reason){
 const labels={paused:['Access paused','Owen has paused your access. Contact Owen if you need to use the desk.'],blocked:['Access unavailable at this time','This time is blocked for your account. Access will return when your schedule allows it.'],outside_shift:['Outside your allowed hours','You can use the desk during the shifts or dates Owen has allowed. All times are UK time.'],verification:['Verify your email','Follow the verification email from Supabase Auth before using the desk.'],approval:['Awaiting approval','Owen, the administrator, needs to approve access before you can read releases.'],check:['Checking access','Please wait while your account access is checked.'],error:['Access could not be checked','Please check your connection and try again.']};
 const [title,detail]=labels[reason]||labels.error;
 message('#waiting-title',title);message('#waiting-detail',detail);message('#waiting-message','');$('#verification-note').hidden=!['approval','verification'].includes(reason);$('#waiting').hidden=false;
}
function armAccessCheck(status){
 clearTimeout(accessTimer);state.access=status;
 if(!status.allowed)return;
 // Use the server's interval and a monotonic timer, never the device's wall clock.
 const delay=Math.max(50,Math.min(30000,Date.parse(status.check_at)-Date.parse(status.server_now)-(status.checkElapsed||0)));
 state.accessMonotonicUntil=performance.now()+delay;state.accessWallUntil=Date.now()+delay;
 accessTimer=setTimeout(()=>{if(!state.session||recovery)return;++authVersion;clearDesk();showAccessMessage('check');refreshAccountAccess();},Number.isFinite(delay)?delay:0);
}
async function refreshAccountAccess(){
 if(accessPollBusy||!state.session||recovery)return;accessPollBusy=true;const version=authVersion;
 try{
  const [member,status]=await Promise.all([membership(),accessStatus()]);if(version!==authVersion)return;
  if(!member?.approved||!status.allowed){++authVersion;clearDesk();state.member=member;state.access=status;showAccessMessage(status.reason);return;}
  if($('#desk').hidden){await onSession(state.session);return;}
  const permissionsChanged=state.member?.can_write_web!==member.can_write_web||state.member?.can_write_copy!==member.can_write_copy;
  state.member=member;armAccessCheck(status);if(permissionsChanged){clearWriting();renderRows();}await loadWritingUsage();
 }catch{if(version!==authVersion)return;++authVersion;clearDesk();showAccessMessage('error');}
 finally{accessPollBusy=false;}
}
async function onSession(session){const version=++authVersion;clearDesk();state.session=session;$('#account').hidden=!session;$('#waiting').hidden=true;$('#auth').hidden=!!session&&!recovery;message('#email',session?.user.email||'');if(!session||recovery)return;
 try{const [member,status]=await Promise.all([membership(),accessStatus()]);if(version!==authVersion)return;state.member=member;state.access=status;
 if(!member?.approved||!status.allowed){showAccessMessage(status.reason);return;}
 armAccessCheck(status);
 ...
 }catch(e){if(version!==authVersion)return;clearDesk();showAccessMessage('error');message('#waiting-message',e.message);}}
...
function queryFor(){let q=client.from('police_releases').select('*',{count:'exact'}).order('published_at',{ascending:false}).order('id');
 ...
 return q;}
async function loadReleases(more=false,quiet=false){if(!state.member?.approved||!state.access?.allowed)return;if(more&&state.busy)return;const version=++state.version;state.busy=true;$('#more').disabled=true;$('#error').hidden=true;
...
function renderRows(){const root=$('#releases');root.replaceChildren();message('#result-count',`${state.total.toLocaleString()} releases${state.rows.length<state.total?' · '+state.rows.length+' loaded':''}`);$('#more').hidden=state.rows.length>=state.total;$('#more').disabled=false;
 ...
 const actions=text('button','⋯','story-actions');actions.type='button';actions.setAttribute('aria-label','Story options for '+row.title);actions.setAttribute('aria-haspopup','dialog');
 actions.addEventListener('click',()=>openStoryMenu(row,actions));meta.append(actions);
 article.addEventListener('contextmenu',event=>{event.preventDefault();openStoryMenu(row,actions,{x:event.clientX,y:event.clientY});});
...
async function loadMembers(){const version=authVersion;const {data,error}=await client.from('desk_members').select('*').order('created_at',{ascending:false});if(version!==authVersion||!state.member?.is_admin)return;if(error){message('#admin-message',error.message);return;}const usage=await client.rpc('desk_writing_admin_usage');if(version!==authVersion)return;const used=new Map((Array.isArray(usage.data)?usage.data:[]).map(x=>[x.user_id,x.used]));const root=$('#members');root.replaceChildren();
 ...
 if('access_revision' in member)line.append(accessControls(member,version));else message('#admin-message','Install the shift-access database update to enable schedules.');
 ...
 root.append(line);}}
function accessControls(member,version){
 const section=text('div','','access-controls');
 if(member.is_admin){section.append(text('small','Administrator access is always available; pause and shift restrictions apply to user accounts.'));return section;}
 const status=text('strong',member.access_paused?'Paused':member.access_scheduled?'Allowed periods only':'Normal access');
 const pause=text('button',member.access_paused?'Resume access':'Pause access');pause.type='button';pause.className='access-pause';
 const feedback=text('p','','access-feedback');feedback.setAttribute('role','status');
 async function save(paused,scheduled,rules,button){button.disabled=true;feedback.textContent='Saving…';try{const {error}=await client.rpc('desk_set_access',{target:member.user_id,paused,scheduled,rules,expected_revision:member.access_revision});if(version!==authVersion)return;if(error)throw error;message('#admin-message','Access settings saved for '+member.email+'.');await loadMembers();}catch(e){feedback.textContent=e.message||'Access settings could not be saved.';button.disabled=false;}}
 pause.onclick=()=>save(!member.access_paused,member.access_scheduled,member.access_rules,pause);
 section.append(status,pause);
 const details=document.createElement('details');details.className='access-editor';details.append(text('summary',`Shift and date rules (${member.access_rules.length})`));
 const form=document.createElement('form');
 const gateLabel=text('label','','access-gate'),gate=document.createElement('input');gate.type='checkbox';gate.checked=member.access_scheduled;gateLabel.append(gate,text('span','Only allow access during allowed periods'));form.append(gateLabel);
 form.append(text('p','UK time (Europe/London). Unticked means normal access except blocked periods. Blocked periods always win. Weekly shifts can run past midnight. Resume keeps these rules.','muted fine'));
 const rows=text('div','','access-rules');let editors=[];
 const labelled=(caption,input,parent)=>{const label=text('label',caption);label.append(input);parent.append(label);return input;};
 const select=(items,value)=>{const input=document.createElement('select');for(const [val,label] of items){const option=text('option',label);option.value=val;input.append(option);}input.value=value;return input;};
 function add(rule){
  const row=text('fieldset','','access-rule');row.append(text('legend',rule.kind==='weekly'?'Weekly period':'One-off dates'));
  const effect=labelled('Access',select([['allow','Allowed'],['block','Blocked']],rule.effect),row);
  let day;if(rule.kind==='weekly')day=labelled('Starting day',select(['Monday','Tuesday','Wednesday','Thursday','Friday','Saturday','Sunday'].map((x,i)=>[String(i+1),x]),String(rule.day||1)),row);
  const start=document.createElement('input'),end=document.createElement('input');start.type=end.type=rule.kind==='weekly'?'time':'datetime-local';start.required=end.required=true;start.step=end.step='60';start.value=rule.start||'';end.value=rule.end||'';
  labelled('From (UK)',start,row);labelled('Until (UK)',end,row);
  if(rule.kind==='weekly')row.append(text('small','An earlier end time means the following day. For a full day, add two periods.'));
  const remove=text('button','Remove period','text-button');remove.type='button';row.append(remove);
  const editor={row,value:()=>({kind:rule.kind,effect:effect.value,...(day?{day:Number(day.value)}:{}),start:start.value,end:end.value})};editors.push(editor);
  remove.onclick=()=>{row.remove();editors=editors.filter(x=>x!==editor);};rows.append(row);
 }
 member.access_rules.forEach(add);form.append(rows);
 const buttons=text('div','','access-buttons');for(const [caption,kind] of [['Add weekly period','weekly'],['Add date range','dates']]){const button=text('button',caption);button.type='button';button.onclick=()=>{if(editors.length>=100){feedback.textContent='Use no more than 100 periods.';return;}add({kind,effect:'allow',day:1,start:kind==='weekly'?'09:00':'',end:kind==='weekly'?'17:00':''});};buttons.append(button);}
 const submit=text('button','Save access schedule');submit.type='submit';buttons.append(submit);form.append(buttons);
 form.onsubmit=event=>{event.preventDefault();save(member.access_paused,gate.checked,editors.map(x=>x.value()),submit);};details.append(form);section.append(details,feedback);return section;
}
function hasWritingAccess(){return !!(state.access?.allowed&&state.member?.approved&&(state.member.can_write_web||state.member.can_write_copy));}
...
async function copyStory(value,label){try{await navigator.clipboard.writeText(value);message('#story-menu-message',label+' copied.');}catch{message('#story-menu-message','Could not copy automatically. Check clipboard permissions and try again.');}}
function openStoryMenu(row,trigger,point){
 const menu=$('#writing-menu'),canWrite=hasWritingAccess(),sourceReady=row.body_status==='available'&&row.body?.trim().length>=120;
 writingMenuTrigger=trigger;message('#writing-story',row.title);message('#story-menu-message','');
 $('#copy-story-headline').onclick=()=>copyStory(row.title,'Headline');
 const copyText=$('#copy-story-text');copyText.disabled=row.body_status!=='available'||!row.body?.trim();copyText.onclick=()=>copyStory(row.body.trim(),'Release text');
 $('#writing-actions').hidden=!canWrite;
 message('#writing-source-note',sourceReady?'Choose a draft to write.':'Full release text is needed before a draft can be written.');
 for(const kind of ['web','copy']){const btn=$('#write-'+kind);btn.disabled=!canWrite||!state.member['can_write_'+kind]||!sourceReady;btn.onclick=()=>{menu.close();openDraft(row,kind);};}
 if(canWrite)renderWritingUsage();menu.style.left='';menu.style.top='';menu.style.margin='';menu.showModal();
...
async function generateDraft(record){
 ...
 try{
  ...
  if(version!==authVersion)return;
  const currentAccess=await accessStatus();if(version!==authVersion)return;if(!currentAccess.allowed){++authVersion;clearDesk();showAccessMessage(currentAccess.reason);return;}armAccessCheck(currentAccess);
...
$('#create-account-form').onsubmit=async event=>{event.preventDefault();if(!state.member?.is_admin)return;const button=$('#create-account-submit'),version=authVersion;button.disabled=true;message('#create-account-message','Creating account…');try{const email=$('#new-account-email').value.trim(),password=$('#new-account-password').value;const {data,error}=await client.functions.invoke('provision-user',{body:{email,password}});if(version!==authVersion)return;if(error){let detail=error.message;try{detail=(await error.context.json()).error||detail;}catch{}throw new Error(detail);}if(data?.error)throw new Error(data.error);$('#create-account-form').reset();message('#create-account-message','Account created and approved. Share the temporary password securely; it was not emailed.');await loadMembers();}catch(e){message('#create-account-message',e.message||'The account could not be created.');}finally{$('#new-account-password').value='';button.disabled=false;}};
$('#open-password').onclick=()=>{if(!state.session)return;$('#password-form').reset();message('#password-message','');$('#password-dialog').showModal();};
$('#close-password').onclick=()=>$('#password-dialog').close();
$('#password-form').onsubmit=async event=>{event.preventDefault();const next=$('#new-password').value;if(next!==$('#confirm-password').value){message('#password-message','The passwords do not match.');return;}const button=$('#change-password-submit');button.disabled=true;message('#password-message','Updating password…');try{const {error}=await client.auth.updateUser({password:next});if(error)throw error;$('#password-form').reset();message('#password-message','Password changed.');}catch(e){message('#password-message',e.message||'The password could not be changed.');}finally{button.disabled=false;}};
...
async function init(){if(!config?.supabaseUrl||!config?.supabasePublishableKey){$('#setup').hidden=false;document.querySelectorAll('#auth button,#auth input').forEach(el=>el.disabled=true);return;}
 ...
 client.auth.onAuthStateChange((event,session)=>{if(event==='TOKEN_REFRESHED'||event==='USER_UPDATED'){state.session=session;return;}if(event==='PASSWORD_RECOVERY'){recovery=true;$('#auth-email').required=false;$('#auth-email').parentElement.hidden=true;message('#auth-submit','Set new password');$('#forgot').hidden=true;$('#password').autocomplete='new-password';}setTimeout(()=>onSession(session),0);});
 await pollHealth();setInterval(()=>{pollHealth();if(state.access?.allowed&&!state.busy&&!$('#desk').hidden)loadReleases(false,true);},30000);
 setInterval(refreshAccountAccess,10000);setInterval(renderHealth,5000);setInterval(updateReleaseTimes,1000);
 const recheckOnReturn=()=>{if(!state.session||recovery)return;if(state.access?.allowed&&(performance.now()>=state.accessMonotonicUntil||Date.now()>=state.accessWallUntil)){++authVersion;clearDesk();showAccessMessage('check');}refreshAccountAccess();};
 document.addEventListener('visibilitychange',()=>{if(!document.hidden)recheckOnReturn();});
 window.addEventListener('pageshow',event=>{if(event.persisted)recheckOnReturn();});
...
if(document.modelContext?.registerTool){
 ...
 const tool={name:'set_police_release_filters',title:'Set police release filters',description:'Change the signed-in desk to all releases, location review, or a chosen combination of patches.',inputSchema:{type:'object',properties:{mode:{type:'string',enum:['all','patches','review','outside']},patchIds:{type:'array',items:{type:'string'}}},required:['mode'],additionalProperties:false},annotations:{readOnlyHint:false,untrustedContentHint:false},async execute(input){
  if(!state.member?.approved||!state.access?.allowed)throw new Error('Sign in with an approved account first.');
