...
const PAGE=100, state={mode:'all',selected:new Set(),rows:[],total:0,patches:[],member:null,session:null,version:0,busy:false,health:null,healthAt:0,source:'police'};
...
function renderHealth(){const online=healthState(state.health,state.healthAt);$('#health').className='health '+(online?'online':'offline');message('#health-label',online?'Online':'Offline');const last=state.source==='ldrs'?state.health?.last_success_at:state.health?.last_data_at;message('#health-detail',last?'Last update '+date(last):client?'No collector updates yet':'Not connected yet');$('#health').title=online?'The '+(state.source==='ldrs'?'LDRS feed':'police collector')+' is sending updates.':'Updates are not arriving. Stored stories remain available.';}
async function pollHealth(){try{const {data,error}=await client.rpc(state.source==='ldrs'?'desk_ldrs_health':'desk_collector_health');if(error)throw error;state.health=data;state.healthAt=Date.now();}catch{state.health=null;}renderHealth();if(!$('#desk').hidden){let notes=[];const h=state.health;if(!healthState(h,state.healthAt))notes.push(state.source==='ldrs'?'LDRS updates are not arriving. Stored stories remain available.':'The police collector is offline. You are viewing stored releases.');if(state.source==='ldrs'&&h?.failed_sources?.length)notes.push(`${h.failed_sources.length} regional LDRS feed${h.failed_sources.length===1?'':'s'} failed on the latest check: ${h.failed_sources.join(', ')}.`);if(state.source==='ldrs'&&h?.capped_sources?.length)notes.push(`Some regional feeds returned more than 500 recent stories: ${h.capped_sources.join(', ')}. The sync limit may need adjusting.`);if(state.source==='police'&&h?.failed_forces)notes.push(`${h.failed_forces} of ${h.force_count} source feeds did not update on the latest check.`);if(state.source==='police'&&h?.pending_releases)notes.push(`${h.pending_releases} releases are waiting to upload.`);message('#sync-status',notes.join(' '));$('#sync-status').hidden=!notes.length;}}
...
function queryFor(){let q=client.from(state.source==='ldrs'?'ldrs_stories':'police_releases').select('*',{count:'exact'}).order('published_at',{ascending:false}).order('id');
 const hours=Number($('#period').value);if(hours)q=q.gte('published_at',new Date(Date.now()-hours*3600000).toISOString());
 if($('#source-filter').value){if(state.source==='ldrs')q=q.contains('source_regions',[$('#source-filter').value]);else q=q.eq('force',$('#source-filter').value);}
 ...
 return q;}
function setSource(source){if(!['police','ldrs'].includes(source)||source===state.source)return;state.source=source;state.rows=[];state.total=0;state.busy=false;state.version++;$('#police-tab').classList.toggle('active',source==='police');$('#ldrs-tab').classList.toggle('active',source==='ldrs');$('#police-tab').setAttribute('aria-selected',String(source==='police'));$('#ldrs-tab').setAttribute('aria-selected',String(source==='ldrs'));message('#feed-eyebrow',source==='ldrs'?'LOCAL DEMOCRACY REPORTING':'POLICE RELEASES');$('#release-feed').setAttribute('aria-label',source==='ldrs'?'LDRS stories':'Police releases');$('#source-filter-label').textContent=source==='ldrs'?'Region':'Force';$('#source-filter').replaceChildren(text('option',source==='ldrs'?'All LDRS regions':'All forces'));$('#source-filter').value='';
 const list=source==='ldrs'?'./ldrs-regions.json':'./forces.json';fetch(list).then(r=>{if(!r.ok)throw new Error('Filter list unavailable');return r.json();}).then(items=>{if(state.source!==source)return;for(const item of items){if(source==='police'&&(item.type||'police')!=='police')continue;const option=text('option',source==='ldrs'?item:item.name);option.value=source==='ldrs'?item:item.key;$('#source-filter').append(option);}}).catch(()=>{});
 message('#result-count','Loading stories…');savePreferences();renderHealth();renderRows();loadReleases();pollHealth();}
...
function renderRows(){const root=$('#releases');root.replaceChildren();message('#result-count',`${state.total.toLocaleString()} releases${state.rows.length<state.total?' · '+state.rows.length+' loaded':''}`);$('#more').hidden=state.rows.length>=state.total;$('#more').disabled=false;
 ...
 const sourceLabel=state.source==='ldrs'?(row.source_regions||[]).join(', ')||'LDRS':row.force_name;
 const meta=text('div','','release-meta');meta.append(text('strong',sourceLabel),releaseTime(row.published_at));banner.append(meta);
...
function renderMap(){if(!map)return;markers.clearLayers();const key=state.mode+'|'+[...state.selected].sort().join(',')+'|'+$('#show-overlays').checked;
 ...
 const popup=document.createElement('div'),link=text('a',row.title);const url=safeLink(row.url);if(url){link.href=url;link.target='_blank';link.rel='noopener noreferrer';}popup.append(link,text('small',`${state.source==='ldrs'?(row.source_regions||[]).join(', '):row.force_name} · ${loc.name} (inferred)`),releaseTime(row.published_at));
...
$('#reset').onclick=()=>{state.mode='all';state.selected.clear();$('#query').value='';$('#source-filter').value='';$('#period').value='24';changed(true);};
$('#patch-search').oninput=renderPatches;
$('#query').oninput=()=>{clearTimeout(searchTimer);searchTimer=setTimeout(()=>loadReleases(),350);};
$('#period').onchange=()=>loadReleases();$('#source-filter').onchange=()=>loadReleases();$('#more').onclick=()=>loadReleases(true);
$('#refresh').onclick=()=>{loadReleases();pollHealth();if(state.member?.is_admin)loadMembers();};
$('#police-tab').onclick=()=>setSource('police');$('#ldrs-tab').onclick=()=>setSource('ldrs');
...
async function init(){if(!config?.supabaseUrl||!config?.supabasePublishableKey){$('#setup').hidden=false;document.querySelectorAll('#auth button,#auth input').forEach(el=>el.disabled=true);return;}
 ...
 try{client=window.supabase.createClient(config.supabaseUrl,config.supabasePublishableKey);const forces=await (await fetch('./forces.json')).json();for(const f of forces){if((f.type||'police')!=='police')continue;const option=text('option',f.name);option.value=f.key;$('#source-filter').append(option);}
