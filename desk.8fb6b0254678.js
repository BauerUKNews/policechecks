...
function clearDesk(){if($('#filter-dialog').open)$('#filter-dialog').close();$('#mobile-admin-tools').hidden=true;state.rows=[];state.member=null;state.version++;state.busy=false;$('#desk').hidden=true;$('#releases').replaceChildren();$('#members').replaceChildren();if(markers)markers.clearLayers();syncResponsiveLayout();}
