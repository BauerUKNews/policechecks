...
export function minutesAgo(published, now = Date.now()) {
 ...
 const unit = (n, name) => `${n.toLocaleString('en-GB')} ${name}${n === 1 ? '' : 's'}`;
 if (minutes < 1) return 'Just now';
 if (minutes < 60) return unit(minutes, 'minute') + ' ago';
 if (minutes < 1440) { const h = Math.floor(minutes / 60), m = minutes % 60; return unit(h, 'hour') + (m ? ' ' + unit(m, 'minute') : '') + ' ago'; }
 const days = Math.floor(minutes / 1440);
 if (days < 7) return unit(days, 'day') + ' ago';
 if (days < 365) return unit(Math.floor(days / 7), 'week') + ' ago';
 return unit(Math.floor(days / 365), 'year') + ' ago';
}
...
export function isForceAreaOnly(row) {
 return row.location_status==='review' && !row.locations?.length && !!row.patch_ids?.length;
}
// Parse labelled web fields without copying diary/CTA/slug sections into the CMS.
export function webDraftSections(value){
 const sections={},matches=[...String(value||'').matchAll(/^\s*(?:#{1,3}\s*)?(?:\*\*)?(KEY DATES|CALL TO ACTION|HEADLINE|SUBHEADING|URL SLUG OVERRIDE|BODY TEXT)(?:\*\*)?:?\s*$/gm)];
 for(let i=0;i<matches.length;i++)sections[matches[i][1]]=value.slice(matches[i].index+matches[i][0].length,matches[i+1]?.index??value.length).trim();
 if(!sections.HEADLINE||!sections.SUBHEADING||!sections['BODY TEXT'])return null;
 return {headline:sections.HEADLINE,subheading:sections.SUBHEADING,body:sections['BODY TEXT']};
}
